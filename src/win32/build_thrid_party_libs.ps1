...
if ($msysBash) {
    ...
    # libvpx configure must run under MSYS2; --target=x86-win32-vs17 emits a VS
    # solution, then msbuild produces vpxmd.lib (the /MD variant).
    $cfg = @"
...
"@
    & $msysBash -lc $cfg
    if ($LASTEXITCODE -ne 0) { throw "libvpx configure (msys2) failed" }
    & msbuild (Join-Path $vpxBuild "vpx.sln") /p:Configuration=Release /p:Platform=Win32
    if ($LASTEXITCODE -ne 0) { throw "libvpx msbuild failed" }
    ...
    if (-not $vpxLib) { throw "could not find built vpx lib" }
...
MSYS2 not found. Build libvpx manually, then copy the /MD lib to third_party_libs\vpx.lib:
  ...
  # in an MSYS2 shell, with yasm/nasm + perl on PATH:
  ...
  ../../third_party/libvpx/configure --target=x86-win32-vs17 \
      ...
      --disable-vp8-encoder --disable-vp9-encoder --enable-static-msvcrt
  # then from a VS command prompt:
  msbuild vpx.sln /p:Configuration=Release /p:Platform=Win32
